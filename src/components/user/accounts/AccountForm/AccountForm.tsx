...
export function AccountForm( { handleAddAccount }: AccountFormProps) {
    ...
    return (
        <form onSubmit={(e : React.FormEvent<HTMLFormElement>) => handleAddAccount(e, newAccountName, newAccountType)} className="flex items-center space-x-2">
            ...
            className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block sm:text-sm border-gray-300 rounded-md bg-background text-account-text border border-border"
            >
            <option value="Checking">Checking</option>
            <option value="Savings">Savings</option>
            </select>
            <button type="submit" className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-button-text bg-button hover:bg-button-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
